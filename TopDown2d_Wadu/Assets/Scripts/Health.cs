...
public class Health : NetworkBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
   {
    ...
    private void OnHealthChanged(int previousValue, int newValue)
    {
        ...
        // ✅ 核心修改：如果是 AI 死了，强行把 victimId 记作 999
        if (gameObject.CompareTag("AI"))
        {
            victimId = 999;
        }
        Debug.Log($"玩家 {victimId} 血量: {newValue}");

        //如果血量归零，且我是服务器，且目前还没死，就执行死亡逻辑
        if (newValue <= 0 && IsServer && !isDead.Value)
        {
            Debug.Log($"玩家 {victimId} 挂了！");
    ...
    private IEnumerator RespawnRoutine()
    {
        ...
        //如果是AI，强行把victimId 记作999
        if (gameObject.CompareTag("AI"))
        {
            victimId = 999;
        }
     
        Debug.Log($"玩家 {victimId} 死亡，3秒后复活...");
